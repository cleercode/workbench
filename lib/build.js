...
function compileJade(srcPath, destPath, options) {
  ...
  if (layout === true || layout === undefined) {
    jade.renderFile(cwd + '/' + srcPath, options, function(err, html) {
      if (err) throw err;
      options.locals = options.locals || {};
      options.locals.body = html;
      _compileJade('source/layouts/layout.jade', destPath, options);
    });
  }
  else {
...
function _compileJade(srcPath, destPath, options) {
  jade.renderFile(cwd + '/' + srcPath, { locals: options.locals }, function(err, html) {
    if (err) throw err;
    fs.writeFile(cwd + '/' + destPath, html, function(err) {
      if (err) throw err;
    });
  });
}
...
function compileStylus(srcPath, destPath) {
  fs.readFile(cwd + '/' + srcPath, 'utf8', function(err, str) {
    if (err) throw err;
    stylus(str).include(nib.path).render(function(err, css) {
      fs.writeFile(cwd + '/' + destPath, css, function(err) {
        if (err) throw err;
        console.log('Compiled ' + srcPath + ' to ' + destPath);
      });
    });
